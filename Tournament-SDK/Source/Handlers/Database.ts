  ...
  try {
    ...
    const components = [
      {
        ...
        components: [
          {
            type: 10,
            content: `<@&1459456963091107902>\n\n<:Core:1461070139608010753> - A new **Classic Tournament** has been created on ***StumbleCore <:Core:1461070139608010753> ***`,
          },
        ],
        accessory: {
          type: 11,
    ...
    if (tournament.Phases && tournament.Phases.length > 0) {
      tournament.Phases.forEach((phase: any, index: number) => {
        ...
        let phaseContent = `<:Core:1461070139608010753> - Phase ${index + 1}: **${phaseTypeName}** <:Core:1461070139608010753>\n`;

        if (phase.MaxTeams) {
          phaseContent += `<:Dot:1460183082144632832> Max Teams: **${phase.MaxTeams}**\n`;
        }

        phaseContent += `<:Dot:1460183082144632832> Maps: **${mapNames}**\n`;

        if (phase.RoundCount) {
          phaseContent += `<:Dot:1460183082144632832> Rounds: **${phase.RoundCount}**\n`;
        }

        if (phase.GroupCount && phase.GroupCount > 1) {
          phaseContent += `<:Dot:1460183082144632832> Passing Teams: **${phase.GroupCount}**\n`;
        }
    ...
    const webhookUrl =
      WEBHOOK_URI.replace("https://discord.com/api/webhooks/1463188232530559039/s_olSSumX_rGCZb6t-yCmqoJYtvhyLCT_6h5dE9NKnL073-m-NUEVilul7IAxVyVVnpB", "https://discord.com/api/webhooks/1463188232530559039/s_olSSumX_rGCZb6t-yCmqoJYtvhyLCT_6h5dE9NKnL073-m-NUEVilul7IAxVyVVnpB") +
